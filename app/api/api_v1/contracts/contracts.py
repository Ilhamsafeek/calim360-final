...
async def generate_contract_stream(
    ...
    """Stream contract generation in real-time using Server-Sent Events"""
    from fastapi.responses import StreamingResponse
    import asyncio
    import json
    
    async def event_generator():
        """Generator function for streaming contract content"""
        try:
            logger.info("🎬 Starting streaming contract generation")
            
            from app.services.claude_service import claude_service
            
            # Extract request data (same as /ai-generate)
            contract_title = request.get("contract_title", "AI Generated Contract")
            contract_type = request.get("contract_type", "Service Agreement")
            profile_type = request.get("profile_type", "client")
            parties = request.get("parties", {})
            ...
            clause_details = {
                ...
                "payment_terms": "Payment Terms - Detailed payment schedule and conditions",
                "timeline": "Timeline and Milestones - Project schedule with key deliverables",
                "back_to_back": "Back-to-Back Terms - Terms shall flow down from main contract",
                "insurance": "Insurance Requirements - Comprehensive insurance coverage",
                ...
                "termination": "Termination Clause - Conditions and procedures for contract termination",
                "arbitration": f"Arbitration Clause - Disputes resolved through arbitration under {jurisdiction} law",
            ...
            for clause in selected_clauses:
                clause_key = clause.get('key', '')
                if clause.get('enabled', False) and clause_key in clause_details:
                    selected_clause_descriptions.append(clause_details[clause_key])
            ...
            # Send initial status
            yield f"data: {json.dumps({'type': 'status', 'message': 'Connecting to AI...'})}\n\n"
            await asyncio.sleep(0.1)
            
            # Stream contract generation
            full_content = ""
            yield f"data: {json.dumps({'type': 'status', 'message': 'AI is generating your contract...'})}\n\n"
            
            async for chunk in claude_service.generate_contract_stream(
                contract_type=contract_type,
                party_a=party_a,
                party_b=party_b,
                jurisdiction=jurisdiction,
            ...
            # Save to database
            yield f"data: {json.dumps({'type': 'status', 'message': 'Saving contract to database...'})}\n\n"
            ...
            # Send completion
            yield f"data: {json.dumps({'type': 'complete', 'contract_id': contract_id, 'contract_number': contract_number, 'word_count': word_count})}\n\n"
