...
async def get_comments(
    ...
    query = text("""
        SELECT cc.id, cc.user_id, CONCAT(u.first_name, ' ', u.last_name) as author,
               cc.comment_text, cc.selected_text, cc.is_resolved, cc.created_at
        FROM contract_comments cc
        INNER JOIN users u ON cc.user_id = u.id
        WHERE cc.contract_id = :cid AND cc.is_resolved = 0
        ORDER BY cc.created_at DESC
    """)
    result = db.execute(query, {"cid": contract_id})
    comments = []
    for row in result:
        comments.append({
            "id": row[0], "user_id": row[1], "author": row[2],
            "comment_text": row[3], "selected_text": row[4],
            "is_resolved": bool(row[5]), "created_at": row[6].isoformat() if row[6] else None
        })
...
async def delete_comment(
    ...
    check = text("SELECT user_id FROM contract_comments WHERE id = :cid")
    result = db.execute(check, {"cid": comment_id}).fetchone()
    if not result:
        raise HTTPException(status_code=404, detail="Comment not found")
    if result[0] != current_user.id:
        raise HTTPException(status_code=403, detail="You can only delete your own comments")
    
    db.execute(text("DELETE FROM contract_comments WHERE id = :cid"), {"cid": comment_id})
    db.commit()
    return {"success": True}